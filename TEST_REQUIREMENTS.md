...
This test project evaluates a **C# (.NET) developer's** ability to work with HTTP APIs (ASP.NET Core), data structures, error handling, and best practices. The project consists of a three-tier architecture where React calls Node.js, which calls the C# backend.
...
Node.js Backend (port 3000) – API Gateway
    ↓
C# Backend (port 8080) – Data Source
```

## Current State

The project is set up with:
- **C# Backend**: ASP.NET Core minimal API serving users and tasks data
- **Node.js Backend**: API gateway that proxies requests to the C# backend
...
1. **Fork/Clone the repository** (or use the provided `csharp-test/` folder)
2. **Set up the development environment**
   - Ensure .NET 8 SDK is installed
   - Ensure Node.js 16+ is installed
   - Install dependencies for Node.js and React
3. **Run all three services (inside `csharp-test/`)**
...
**Task**: Implement a POST endpoint in the C# backend to create new users.
...
**Task**: Implement a POST endpoint in the C# backend to create new tasks.
...
**Task**: Add structured logging for all HTTP requests in the C# backend.
...
You can adapt the Go/Java advanced requirements to C# if you have time:
...
- **C#/.NET Best Practices**: idiomatic C#, use of ASP.NET Core features
